...
# 부록 · 마이리얼트립 행 중복 (2026-09-14)

## 증상 두 가지, 뿌리는 하나

1. 성인과 아동을 섞어 예약하면 행이 여러 개 생긴다
2. 예약대기였다가 수동으로 예약확정하면 행이 또 생긴다

마이리얼트립은 **인원 1명당 예약번호(EXP)를 따로 발급**한다.
성인 2 + 아동 1 이면 `확정대기` 3통 + `확정완료` 3통이 오고, cron 은 order_id 로만
매칭하므로 행을 3개 만든다. 운영자가 한 행으로 합치고 나머지를 지우면,
**지워진 예약번호의 확정완료 메일이 다시 와서 행을 되살린다.** 둘 다 같은 원인이다.

메일에는 주문 단위 식별자가 없다(파트너센터에만 `주문번호` 가 있다).
예약번호·상품명·옵션명(성인/아동)·여행일·여행자가 전부다.

## 처리

INSERT 하기 전에 **같은 이름 + 같은 여행일 + 취소 아님** 인 M 예약을 찾는다.

| 찾은 행 | 처리 |
|---|---|
| 1건 | 행을 만들지 않고 note 에 `[동일 예약 합침: EXP-…]` 기록 + **Discord 알림** |
| 0건 | 평소대로 INSERT |
| 2건 이상 | 어느 쪽인지 알 수 없으므로 **손대지 않는다** |

같은 이름·같은 날의 **별개 예약**일 수도 있어서 조용히 버리지 않고 반드시 알린다.
인원(`pax`)은 자동으로 더하지 않는다 — 운영자가 이미 맞게 적어 둔 값을 망가뜨리지
않기 위해서다. 대신 알림에 현재 인원을 실어 확인을 요청한다.

```
🧩 [인원 추가] 같은 예약의 다른 예약번호 — 인원 확인 필요
   📝 기존 예약확정 행에 합쳤습니다 (현재 인원 3명).
      마이리얼트립은 1인당 예약번호가 따로 발급되니 인원이 맞는지 확인해주세요.
```

## 실측

최근 14일 확정 계열 407통으로 시뮬레이션:

```
365  예약번호로 매칭 (기존과 동일)
 29  🧩 기존 행에 합침   ← 예전이면 행이 29개 더 생겼을 것
 12  신규 INSERT
  1  후보 2건 이상 → 보류
```

하루 두 건꼴로 생기던 수동 병합 작업이 없어진다.
