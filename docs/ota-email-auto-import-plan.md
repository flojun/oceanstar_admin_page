...
### 시간 → option 매핑 (`tour_settings` 실제 시각 기준)
```
문자열에 1부/2부/3부/sunset/선셋 이 직접 있으면 그대로 사용
없으면 시작시각으로: 07:00–10:59 → 1부 / 11:00–14:59 → 2부 / 15:00~ → 3부
시각도 못 찾으면 option = '' (운영자가 직접 채움)
```
...
- 기존 `sendDiscordUrgentAlert()` 를 **그대로 재사용** (title 이 파라미터라 코드 변경 0줄).
  고객명·투어일·옵션·출처·인원·예약번호가 자동으로 embed 에 들어감.
- 취소 알림은 **항상** 보냄 — 취소요청 건은 운영자가 마감하기 전까지 정원을 계속 차지하므로
  빨리 알아야 함.
- 참고: 이 함수는 빨간 embed + `DISCORD_URGENT_ROLE_ID` 멘션이 고정임.
  취소 알림 멘션이 너무 시끄러우면 나중에 `color`/`mention` 옵션 파라미터 추가 (지금은 안 함).
