...
# 시안 B 비교표와 같은 상품·가격
TOURS = [
    ("와이키키 거북이 스노클링",              "1부 07:30-11:30 · 2부 10:30-14:30", "₩151,570", "turtle.jpg"),
    ("선셋·와인 & 와이키키 거북이 스노클링",   "시즌별 시간 변동",                   "₩206,690", "sunset.jpg"),
    ("거북이 스노클링 + 패러세일링 / 제트스키", "패러/제트 9:30-2:00",               "₩289,360", "parasail.jpg"),
    ("[단독] 프라이빗 와이키키 거북이 스노클링", "2시간 완전 단독 · 1-10명 / 팀",      "₩1,653,480 ~", "boat_private.webp"),
    ("거북이 스노클링 + 서핑",                "서핑 하루 5회 07:30-14:30",          "₩220,460", "surf.jpg"),
]
...
# 목록 가격은 성인 1인 기준(운영자 요청: 낮은 첫 가격이 예약률을 올린다).
# 처음엔 성인 2명 총액을 크게 보였다. 실제 낼 돈은 인원을 넣으면 오른쪽 요약에
# 계산되어 나온다. 원화는 $×1,377.9 임시값이고 배포 때 환율 자동 계산에 연결한다.
# 프라이빗만 팀 요금이라 1인 값이 없다($1,200, 1-10명).
TOTALS = {
    0: ("\u20a9151,570",     "성인 1인"),
    1: ("\u20a9206,690",     "성인 1인"),
    2: ("\u20a9289,360",     "성인 1인"),
    3: ("\u20a91,653,480 ~", "팀 요금 · 1-10명"),
    4: ("\u20a9220,460",     "성인 1인"),
}
...
.pick a{display:flex;align-items:center;min-height:44px;padding:0 2px;font-size:11.5px;
  ...
  white-space:nowrap}
/* 픽업 줄까지가 숙소 묶음이다. 그 아래 예약자 칸은 다른 묶음이라 칸 사이
   간격(12px)이 아니라 묶음 사이 간격을 준다. */
.pick+.f,.pick+.two{margin-top:24px}
.two+.f{margin-top:12px}
"""
