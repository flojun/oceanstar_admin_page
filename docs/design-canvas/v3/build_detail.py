...
.hero.sunset .hero-in h1 .hl{color:#FFC08A}
/* 선셋 모바일 — 제목 묶음을 가운데로 모아 위로 올리고, 그 아래 빈 띠에
   잔을 부딪치는 사진이 가리지 않고 보이게 한다. */
.hero.sunset .hero-in{top:84px;text-align:center}
.hero.sunset .hero-in h1{margin-top:14px;font-size:36px}
.hero.sunset .hero-in .rev{margin-top:14px}
.hero.sunset .veil{background:linear-gradient(180deg,
