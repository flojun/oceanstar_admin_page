...
/* 소개글은 제목보다 한 단 안쪽에서 시작한다(운영자 요청). 제목과 같은 왼쪽선에
   붙으면 제목의 연장처럼 읽혀 위계가 흐려진다. 들인 자리에 브랜드색 2px 세로선을
   두어, 어긋난 것이 아니라 일부러 들인 단임을 보인다. */
.sh .lede{margin-top:18px;margin-left:34px;padding-left:20px;border-left:2px solid var(--sky)}
.center .sh{max-width:none}
.center .sh .lede{margin-left:auto;margin-right:auto;padding-left:0;border-left:0}
...
.hero.para .veil{background:
  ...
  linear-gradient(100deg,rgba(9,16,22,.30) 0%,rgba(9,16,22,.25) 16%,rgba(9,16,22,.17) 32%,
    rgba(9,16,22,.09) 45%,rgba(9,16,22,.03) 56%,rgba(9,16,22,0) 66%)}
/* 덮개를 옅게 한 만큼 글자 자체에 그림자를 준다. */
.hero.para .hero-in{text-shadow:0 2px 6px rgba(9,16,22,.28),0 2px 26px rgba(9,16,22,.42)}

/* 콤보 — 패러세일링 · 제트스키. 사진 반, 글 반. 두 판은 사진 쪽을 바꿔 리듬을 준다. */
.acts{margin-top:44px;display:grid;gap:28px}
/* 사진 칸을 넓히고 사양 표를 두 단으로 접어 카드 높이를 줄였다. 둘 다 1:1 에
   가깝던 사진 칸을 4:3 가까이 되돌려, 사진 좌우가 덜 잘린다. */
.act{display:grid;grid-template-columns:1.2fr 1fr;background:#fff;border:1px solid var(--line);
  border-radius:22px;overflow:hidden}
.act.rev{grid-template-columns:1fr 1.2fr}
...
.act-spec div:nth-child(-n+2){border-top:1px solid var(--line)}
...
.dlist{margin-top:40px;background:#fff;border:1px solid var(--line);border-radius:22px;
  padding:6px 40px}
.dl{display:grid;grid-template-columns:180px 1fr;align-items:center;min-height:84px;padding:14px 0}
.dl + .dl{border-top:1px solid var(--line)}
.dl h3{font-size:22px}
.dl ul{display:flex;flex-wrap:wrap;column-gap:56px;row-gap:8px}
.dl li{display:inline-flex;align-items:baseline;gap:10px}
.dl li b{font-family:'SUIT',system-ui,sans-serif;font-size:24px;font-weight:800;color:var(--deep);
  letter-spacing:-.01em}
.dl li i{font-style:normal;font-size:14px;font-weight:800;color:var(--food-d)}
.dl li.sun b{color:var(--food-d)}
.dlist + .tnote{margin-top:18px}
...
.tlist{margin-top:30px;margin-left:34px;padding-left:22px}  /* 소개글 글줄과 같은 선에서 시작 */
...
.hop-l{font-size:16px;font-weight:700;line-height:1.45;color:var(--ink);word-break:keep-all}
...
/* 폭 가득 찬 예약 버튼은 글자를 가운데, 화살표 원은 오른쪽 끝에 그대로(운영자). */
.cb .book-pill,.buy-r .book-pill{position:relative;justify-content:center;padding:0 52px}
.cb .book-pill svg,.buy-r .book-pill svg{position:absolute;right:10px;top:50%;transform:translateY(-50%)}
/* 옵션 세 장 — 코스 소개와 같은 가로 넘김. 카드 폭 280px, 다음 카드가 살짝 보이게. */
.combo.n3{margin:16px calc(var(--pad) * -1) 0;padding:2px var(--pad) 16px;display:grid;
  grid-auto-flow:column;grid-auto-columns:280px;gap:12px;overflow-x:auto;
  overscroll-behavior-x:contain;scroll-snap-type:x mandatory;scroll-padding:0 var(--pad);
...
/* 프라이빗 예약 가능 시간 — 데스크탑과 같은 결. 요일 굵게, 시간은 숫자만 세로로. */
.dlist{margin-top:24px;background:#fff;border:1px solid var(--line);border-radius:20px;padding:2px 20px}
.dl{display:grid;grid-template-columns:76px 1fr;gap:12px;align-items:start;padding:18px 0}
.dl + .dl{border-top:1px solid var(--line)}
.dl h3{font-size:17px;line-height:28px}
.dl ul{display:grid;gap:6px}
.dl li{display:flex;align-items:baseline;gap:8px}
.dl li b{font-family:'SUIT',system-ui,sans-serif;font-size:19px;font-weight:800;line-height:28px;
  color:var(--deep)}
.dl li i{font-style:normal;font-size:13px;font-weight:800;color:var(--food-d)}
.dl li.sun b{color:var(--food-d)}
...
.sh + .cs-hint{justify-content:center}  /* 가운데 정렬한 섹션 머리 아래에서는 안내 줄도 가운데 */
...
.dock{position:sticky;bottom:0;z-index:20;display:flex;align-items:center;
  ...
  -webkit-backdrop-filter:blur(14px);backdrop-filter:blur(14px)}  /* 그림자 없이 위 선만(운영자 요청) */
...
def days(mobile):
    """프라이빗 예약 가능 시간. 처음엔 같은 칸 세 장(월요일이 텅 빔), 다음엔 시간 축
    막대표(격자·축·겹침 줄이 많아 번잡)였다. 손님이 알 것은 '무슨 요일, 몇 시' 뿐이라
    장치를 다 걷고 글자 위계만 남긴다. 요일 한 줄에 시간을 나란히, 선셋만 주황 글자."""
    rows = []
    for d, ts in C.DAY_SLOTS:
        ...
        rows.append(f'<div class="dl"><h3>{d}</h3><ul>{items}</ul></div>')
    return (f'<section class="sect">{sh(C.TIME_H2, C.TIME_SUB)}'
            f'<div class="dlist rise">{"".join(rows)}</div>'
