...
| 어드민 | 22개 페이지 로드(로그인으로 튕기지 않음), 사이드바 15개 메뉴 → 정확한 경로, **고객 헤더/카카오 위젯이 어드민에 새지 않음**, (스테이징) 판매중지 토글 → 고객 `/api/settings` 반영 → 원복 |
...
4. 깊이 2(기본): 클릭으로 새로 나타난 요소(예약 모달 안 투어 카드, 리뷰 모달의 별점 등)까지 한 단계 더 들어가 같은 방식으로 누른다.
5. 결과는 `qa/reports/sweep/<프로젝트>/<라우트>.md|json`. 리뉴얼 전 운영 결과를 보관해 두면, 리뉴얼 후 같은 라벨의 판정이 바뀐 것(예: `request` → `none`)을 바로 비교할 수 있다.
...
| 운영 버튼 전수 클릭 결과 보관: `QA_BASE_URL=https://www.oceanstarhi.com npm run qa:e2e:sweep` → `qa/reports/sweep/` 를 `qa/baseline/sweep/` 로 복사해 커밋 | ☐ 전체 라우트는 로컬에서 1회 실행 권장 (§10 에 `/kr` 결과) |
...
| 검사 | 결과 |
|---|---|
| `npm run qa:static` | 통과 (tsc 0, ESLint 264=기준선, 정적 감사 차단 0 / warn 3, 파서 5종 통과) |
...
| 고객 페이지 스모크 (데스크탑·모바일 24개) | 전부 통과 |
| 고객 흐름 (예약 계약 ko/en, 검증, 언어 전환, 내 예약 관리, 리뷰, 맛집 QR, 연락 채널) | 12 통과, 1 건너뜀(테스트 예약 필요) |
| 버튼 전수 클릭 `/kr` 데스크탑 | `qa/reports/sweep/public-desktop/kr.md` (아래 부록 B) |
