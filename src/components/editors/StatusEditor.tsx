...
export default function StatusEditor<R, SR>({ row, column, onRowChange, onClose }: RenderEditCellProps<R, SR>) {
    ...
    const options = [
        ...
        { value: "안내필요", label: "안내필요 (Needs Info)", color: "text-purple-600" },
            ...
            {createPortal(
                ...
                >
                    {options.map((opt) => (
                        ...
                        >
                            <span className={`w-2 h-2 rounded-full ${opt.value === '예약확정' ? 'bg-green-500' : opt.value === '예약대기' ? 'bg-yellow-500' : opt.value === '안내필요' ? 'bg-purple-500' : opt.value === '취소요청' ? 'bg-orange-500' : 'bg-red-500'}`} />
