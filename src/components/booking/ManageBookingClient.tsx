...
export default function ManageBookingClient({ lang }: { lang: Language }) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-10 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="bg-blue-600 py-6 px-6 text-center">
          <h1 className="text-2xl font-bold text-white">{t('manage.title')}</h1>
          <p className="text-blue-100 text-sm mt-1">{t('manage.subtitle')}</p>
        </div>
          ...
          {!isVerified ? (
            <form onSubmit={handleVerify} className="space-y-5">
              <p className="text-gray-600 text-sm mb-4">
                {t('manage.verify_desc')}
              </p>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('manage.res_num')}</label>
                <input
                  type="text"
                  placeholder={t('manage.res_num_ph')}
                  maxLength={6}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('manage.email')}</label>
                <input
                  type="email"
                  placeholder={t('manage.email_ph')}
              ...
              >
                {isVerifying ? <Loader2 className="w-5 h-5 animate-spin" /> : t('manage.submit_btn')}
              </button>
            </form>
          ) : (
            ...
            <div className="w-full animate-fade-in">
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-8 text-sm text-blue-800 flex items-start shadow-sm break-keep">
                ...
                <p className="leading-relaxed">
                  <span className="font-bold shrink-0">{t('manage.notice_prefix')}</span> {t('manage.notice_text')} <br />
                  <span className="text-xs opacity-90 mt-1 block">{t('manage.notice_sub')}</span>
                </p>
              </div>
