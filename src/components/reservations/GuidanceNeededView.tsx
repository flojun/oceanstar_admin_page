...
export default function GuidanceNeededView() {
    ...
    if (rows.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-gray-400 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                ...
                <p className="text-lg font-medium">안내가 필요한 예약이 없습니다.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4 p-4 bg-gray-50/50 rounded-lg">
            <h2 className="text-lg font-bold text-gray-700 mb-4">안내 필요 ({rows.length}건)</h2>
                ...
                {rows.map((r) => (
                    ...
                    >
                        ...
                        <div className="min-w-0 flex-1 flex flex-wrap items-center gap-x-2 gap-y-0.5">
                            ...
                            <span className="text-[11px] px-1.5 py-0.5 rounded font-medium bg-purple-50 text-purple-700">안내필요</span>
                            <span className="text-[11px] text-gray-400">{r.source}</span>
                            <span className="font-bold text-purple-600 text-xs">{formatDateDisplay(r.tour_date)}</span>
                            <span className="text-xs text-gray-500">{r.pax}</span>
                            <span className="text-xs text-gray-400 truncate max-w-[160px]" title={r.option}>{r.option}</span>
                            <span className="text-xs text-gray-400 truncate max-w-[140px]" title={r.pickup_location}>{r.pickup_location}</span>
                            <span className="text-xs text-gray-400 truncate max-w-[110px]" title={r.contact}>{r.contact}</span>
                        </div>
                        <button
                            onClick={() => markGuided(r)}
                            disabled={processingId === r.id}
                            className="shrink-0 px-2 py-1 text-xs font-bold text-white bg-purple-600 rounded hover:bg-purple-700 disabled:opacity-50"
                        >
